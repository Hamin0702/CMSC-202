//Had to edit/reorder some lines of code to macth the expected output on the document
//For example, the order of messages were wrong or it was using the wrong method (SetLit instead of GetLit)
//For some reason there were only problems for the birthday candle messages
...
int main()
{
  ...
  cout << birth1.SetLit(false) << endl;;
  cout << "Flame: " << birth1.GetLit() << endl; //Uses Parent Class - example of using

  cout << "Everyone sings to you and you try to put out the birthday candle!" << endl;
  cout << birth1.SetLit(false) << endl;; //Uses Child Class - example of replacing
